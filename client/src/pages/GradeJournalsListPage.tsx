...
export default function GradeJournalsListPage() {
  const [journals, setJournals] = useState<Journal[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    api.get('/journals?type=grades')
  ...
  if (loading) {
    return (
      <div className="space-y-6">
        <h2 className="text-2xl font-bold text-slate-900">Журналы успеваемости</h2>
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-3xl font-bold gradient-text">Журналы успеваемости</h2>
        <p className="text-slate-500 mt-1">Все журналы успеваемости</p>
      </div>
      ...
      {journals.length === 0 && (
        <div className="text-center py-12 text-slate-500">Журналы успеваемости не найдены</div>
      )}
    </div>
  );
}
