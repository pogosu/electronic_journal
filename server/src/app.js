...
app.use(cors({
  origin: (origin, callback) => {
    if (isProduction) {
      // В продакшене разрешаем любой origin (тестовый стенд на VPS)
      callback(null, true);
    } else if (!origin || allowedOrigins.includes(origin) || origin.includes('pinggy') || origin.includes('trycloudflare') || origin.includes('ngrok')) {
      callback(null, true);
    } else {
      callback(null, false);
    }
