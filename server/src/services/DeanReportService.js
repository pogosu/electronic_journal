  ...
  async getGroupSummaries() {
    const result = await query(
      `WITH group_debtors AS (
         SELECT s.group_id, COUNT(DISTINCT s.id) as debtor_count
         FROM students s
         JOIN journals j ON j.group_id = s.group_id
         JOIN works w ON w.journal_id = j.id AND w.is_mandatory = true
       ...
       group_attendance AS (
         SELECT s.group_id, COUNT(CASE WHEN a.status = 'present' THEN 1 END) * 100.0 / NULLIF(COUNT(a.id), 0) as attendance_pct
         FROM students s
  ...
  async getGroupDisciplines(groupId) {
    const result = await query(
      `WITH current_semester AS (
       ...
       exam_avg AS (
         SELECT j.discipline_id, COALESCE(AVG(gr.score), 0) as exam_avg_score
         FROM works w
       ...
       discipline_attendance AS (
         SELECT j.discipline_id, COUNT(CASE WHEN a.status = 'present' THEN 1 END) * 100.0 / NULLIF(COUNT(a.id), 0) as attendance_pct
         FROM attendances a
       ...
       group_journals AS (
         ...
         FROM journals j
         WHERE j.group_id = $1 AND j.semester = (SELECT semester FROM current_semester)
         GROUP BY j.discipline_id
       )
       SELECT gd.id, gd.name,
         COALESCE(ea.exam_avg_score, 0) as exam_avg_score,
         COALESCE(da.attendance_pct, 0) as attendance_pct,
         gj.grade_journal_id, gj.attendance_journal_id
    ...
    const result = await query(
      `WITH exam_avg AS (
       ...
       discipline_groups AS (
         SELECT ta.discipline_id, COUNT(DISTINCT ta.group_id) as group_count, COUNT(DISTINCT s.id) as student_count
  ...
  async getStudentSummariesByGroup(groupId) {
    const result = await query(
      `WITH student_progress AS (
         SELECT s.id as student_id, COALESCE(SUM(gr.score) / NULLIF(SUM(w.max_score), 0) * 100, 0) as progress
         FROM students s
         JOIN journals j ON j.group_id = s.group_id
         JOIN works w ON w.journal_id = j.id
         LEFT JOIN grades gr ON gr.work_id = w.id AND gr.student_id = s.id
         WHERE s.group_id = $1
         GROUP BY s.id
       ),
       student_attendance AS (
         SELECT s.id as student_id, COUNT(CASE WHEN a.status = 'present' THEN 1 END) * 100.0 / NULLIF(COUNT(a.id), 0) as attendance_pct
         FROM students s
         LEFT JOIN attendances a ON a.student_id = s.id
         WHERE s.group_id = $1
         GROUP BY s.id
       )
       SELECT u.id as user_id, s.id as student_id, u.full_name,
...
export default new DeanReportService();
