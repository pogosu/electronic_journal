...
export default class StudentRepository {
  ...
  static async getProfile(studentId) {
    const groupRes = await query('SELECT name, admission_year FROM groups WHERE id = (SELECT group_id FROM students WHERE id = $1)', [studentId]);
  ...
  static async getMyGradesFullData(studentId) {
    const result = await query(
      `SELECT 
        j.id as journal_id,
        j.semester,
       ...
       WHERE j.type = 'grades' AND s.id = $1
  ...
  static async getFullStats(studentId) {
